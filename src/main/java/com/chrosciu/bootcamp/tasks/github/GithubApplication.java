...
public class GithubApplication {
    ...
    @SneakyThrows
    private void run() {
        githubClient.getUserRepositories("tyrontundrom")
                .subscribe(s -> log.info("Repository: {}", s));


        githubClient.getUserRepositoryBranches("tyrontundrom", "todo-app")
                .subscribe(s -> log.info("Repository: {}", s),
                        e -> log.warn("Error: ", e),
                        () -> log.info("Completed"),
                        subscription -> {
        ...
        githubClient.getUsersRepositories(Flux.just("tyrontundrom", "chrosciu"))
                .subscribe(s -> log.info("Repository: {}", s));

        githubClient.getAllUserBranchesNames("tyrontundrom")
                .subscribe(s -> log.info("Repository: {}", s));
